...
# def Cal_eval_index(epoch, out, val_target, arr, validation_losses, validation_MAE, validation_RMSE, validation_MAPE, max_X, min_X):
    for item in arr:
        ...
        if (epoch % 500 == 0) & (epoch != 0):
            ...
            for i in range(0, 44):
                # 社区7到商圈2的流量预测
                C_to_B_pred = out_unnormalized[:, 0, :]
                C_to_B_true = target_unnormalized[:, 0, :]
                spatial_pre4.append(C_to_B_pred[i])
                spatial_true4.append(C_to_B_true[i])
                pd.DataFrame(C_to_B_pred).to_csv(
                    "./results_TFAGCN/results_classCB_test_attention/C_to_B_pred" + str(epoch) + ".csv", index=None, header=None)
                pd.DataFrame(C_to_B_true).to_csv(
                    "./results_TFAGCN/results_classCB_test_attention/C_to_B_true" + str(epoch) + ".csv", index=None, header=None)

            pd.DataFrame(spatial_pre4).to_csv("./results_TFAGCN_16_4_attention/spatial_pre4_" + str(epoch) + ".csv", index=None,
                                              header=None)
            pd.DataFrame(spatial_true4).to_csv("./results_TFAGCN_16_4_attention/spatial_true4_" + str(epoch) + ".csv", index=None,
                                               header=None)
            print()
